...
class Order(Base, AsyncAttrs):
    ...
    order_id: Mapped[uuid.UUID] = mapped_column( primary_key=True, nullable=False, index=True, default=uuid.uuid4)
    item_name: Mapped[str] = mapped_column(nullable=False, index=True)
    quantity: Mapped[int] = mapped_column(nullable=False, index=True)
    order_time: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), default=func.now()) 
