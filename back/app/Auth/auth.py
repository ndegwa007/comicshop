...
def create_access_token(data: dict, expires_delta: timedelta | None = None):
    ...
    to_encode.update({"exp": expires, "user_id": str(data["user_id"])})
...
async def get_user_by_id(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    user = (await db.scalars(select(models.User).where(models.User.userid == id))).first()
    return user
...
def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    return {"email": email, "name": payload.get("user")}
